#!/usr/bin/env python3
"""Verify script for M6.9 — Admin Release Status."""
...
def main() -> int:
    print("=" * 60)
    print("M6.9 Verify — Admin Release Status")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "27 passed" in result.stdout:
        print("  OK M6 tests PASS (27/27)")
    else:
    ...
    if version == "0.6.0-alpha9" and "M6.9" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.9")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking admin release route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/release" in api_text:
        print("  OK admin release route PASS")
    else:
        print("  FAILED admin release route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.9 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
