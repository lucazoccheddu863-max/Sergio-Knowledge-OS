#!/usr/bin/env python3
"""Verify script for M6.16 — Operator Manual."""
...
def main() -> int:
    print("=" * 60)
    print("M6.16 Verify — Operator Manual")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "52 passed" in result.stdout:
        print("  OK M6 tests PASS (52/52)")
    else:
    ...
    if version == "0.6.0-alpha16" and "M6.16" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.16")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking operator manual route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/manual" in api_text:
        print("  OK operator manual route PASS")
    else:
        print("  FAILED operator manual route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.16 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
