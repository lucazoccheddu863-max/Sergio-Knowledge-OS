#!/usr/bin/env python3
"""Verify script for M6.2 — Admin Readiness."""
...
def main() -> int:
    print("=" * 60)
    print("M6.2 Verify — Admin Readiness")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "6 passed" in result.stdout:
        print("  OK M6 tests PASS (6/6)")
    else:
    ...
    if version == "0.6.0-alpha2" and "M6.2" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.2")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking readiness endpoint wiring...")
    ...
    if "/api/v1/admin/readiness" in api_text and "/api/v1/admin/readiness" in js_text:
        print("  OK readiness endpoint wiring PASS")
    else:
        print("  FAILED readiness endpoint wiring")
        return 1

    print("\n" + "=" * 60)
    print("M6.2 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
