...
class TestInMemoryEventBus:
    def test_publish_and_subscribe(self) -> None:
        ...
        event = DomainEvent(event_id="evt-1", event_type="test.event", correlation_id="corr-1", payload={"msg": "hello"})
    ...
    def test_unsubscribe(self) -> None:
        ...
        sub.unsubscribe()
        event = DomainEvent(event_id="evt-2", event_type="test.event", correlation_id="corr-2")
        bus.publish(event, "test.topic")
        assert len(received) == 0

    def test_no_subscribers_no_error(self) -> None:
        bus = InMemoryEventBus()
        event = DomainEvent(event_id="evt-3", event_type="test.event", correlation_id="corr-3")
        bus.publish(event, "empty.topic")
    ...
    def test_multiple_subscribers(self) -> None:
        ...
        event = DomainEvent(event_id="evt-4", event_type="test.event", correlation_id="corr-4")
    ...
    def test_handler_exception_does_not_crash_bus(self) -> None:
        ...
        event = DomainEvent(event_id="evt-5", event_type="test.event", correlation_id="corr-5")
