#!/usr/bin/env python3
"""Verify script for M6.14 — Release Readiness Gate."""
...
def main() -> int:
    print("=" * 60)
    print("M6.14 Verify — Release Readiness Gate")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "43 passed" in result.stdout:
        print("  OK M6 tests PASS (43/43)")
    else:
    ...
    if version == "0.6.0-alpha14" and "M6.14" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.14")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking release gate route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/release/gate" in api_text:
        print("  OK release gate route PASS")
    else:
        print("  FAILED release gate route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.14 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
