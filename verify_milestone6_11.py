#!/usr/bin/env python3
"""Verify script for M6.11 — Admin Smoke Check."""
...
def main() -> int:
    print("=" * 60)
    print("M6.11 Verify — Admin Smoke Check")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "33 passed" in result.stdout:
        print("  OK M6 tests PASS (33/33)")
    else:
    ...
    if version == "0.6.0-alpha11" and "M6.11" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.11")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking admin smoke route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/smoke" in api_text:
        print("  OK admin smoke route PASS")
    else:
        print("  FAILED admin smoke route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.11 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
