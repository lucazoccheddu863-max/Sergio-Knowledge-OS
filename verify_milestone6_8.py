#!/usr/bin/env python3
"""Verify script for M6.8 — Admin Backup Console."""
...
def main() -> int:
    print("=" * 60)
    print("M6.8 Verify — Admin Backup Console")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "24 passed" in result.stdout:
        print("  OK M6 tests PASS (24/24)")
    else:
    ...
    if version == "0.6.0-alpha8" and "M6.8" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.8")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking backup console assets...")
    ...
    if "Backup Operations" in html and "/api/v1/admin/backup/create" in js:
        print("  OK backup console assets PASS")
    else:
        print("  FAILED backup console assets missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.8 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
