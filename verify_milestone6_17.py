#!/usr/bin/env python3
"""Verify script for M6.17 — Local Workspace Bootstrap."""
...
def main() -> int:
    print("=" * 60)
    print("M6.17 Verify — Local Workspace Bootstrap")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "56 passed" in result.stdout:
        print("  OK M6 tests PASS (56/56)")
    else:
    ...
    if version == "0.6.0-alpha17" and "M6.17" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.17")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking local bootstrap route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/local/bootstrap" in api_text:
        print("  OK local bootstrap route PASS")
    else:
        print("  FAILED local bootstrap route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.17 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
