#!/usr/bin/env python3
"""Verification script for Milestone 4.8 — Query Orchestrator."""
import subprocess
import sys
...
def ensure_package() -> bool:
    try:
        ...
        setup = PROJECT_ROOT / "setup_milestone4_8.py"
        if setup.exists():
...
def verify_m4_7() -> bool:
    return _check(
        [PROJECT_ROOT / "skos" / "m4" / "infrastructure" / "adapters" / "knowledge_graph" / "inmemory_kg_adapter.py"],
        "M4.7 Baseline",
    )
...
def verify_orchestrator_port() -> bool:
    print(f"\n{'='*60}")
    print("Verifying QueryOrchestratorPort...")
    print(f"{'='*60}")
    try:
        ...
        print("✅ QueryOrchestratorPort: PASSED")
        return True
    except Exception as e:
        print(f"❌ QueryOrchestratorPort: FAILED — {e}")
        return False


def verify_orchestrator_service() -> bool:
    print(f"\n{'='*60}")
    print("Verifying QueryOrchestratorService...")
    print(f"{'='*60}")
    try:
        from skos.m4.application.services.query_orchestrator_service import QueryOrchestratorService
        import inspect
        sig = inspect.signature(QueryOrchestratorService.__init__)
        params = list(sig.parameters.keys())
        assert "semantic_search" in params
        assert "rag_pipeline" in params
        assert "knowledge_graph" in params
        assert "config" in params
        assert "event_bus" in params
        print("✅ QueryOrchestratorService: PASSED")
        return True
    except Exception as e:
        print(f"❌ QueryOrchestratorService: FAILED — {e}")
        return False
...
def main() -> int:
    print(f"\n{'='*60}")
    print("MILESTONE 4.8 — QUERY ORCHESTRATOR")
    print(f"{'='*60}")
    ...
    results = {
        ...
        "M4.7 Baseline": verify_m4_7(),
        "QueryOrchestratorPort": verify_orchestrator_port(),
        "QueryOrchestratorService": verify_orchestrator_service(),
        "Architecture Rules": verify_architecture(),
        "M4.8 Test Suite": run_tests(),
    }
    print(f"\n{'='*60}")
    ...
    print(f"{'='*60}")
    if all_passed:
        print(f"\n🎉 ALL VERIFICATIONS PASSED — M4.8 is READY")
        return 0
