#!/usr/bin/env python3
"""Verify script for M6.15 — Local Launch Preflight."""
...
def main() -> int:
    print("=" * 60)
    print("M6.15 Verify — Local Launch Preflight")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "48 passed" in result.stdout:
        print("  OK M6 tests PASS (48/48)")
    else:
    ...
    if version == "0.6.0-alpha15" and "M6.15" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.15")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking local launch route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/local/launch" in api_text:
        print("  OK local launch route PASS")
    else:
        print("  FAILED local launch route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.15 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
