#!/usr/bin/env python3
"""Verify script for M6.10 — Admin Overview."""
...
def main() -> int:
    print("=" * 60)
    print("M6.10 Verify — Admin Overview")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "30 passed" in result.stdout:
        print("  OK M6 tests PASS (30/30)")
    else:
    ...
    if version == "0.6.0-alpha10" and "M6.10" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.10")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking admin overview route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/overview" in api_text:
        print("  OK admin overview route PASS")
    else:
        print("  FAILED admin overview route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.10 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
