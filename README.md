# Sergio Knowledge OS

**Version:** 0.7.0-alpha12
**Status:** M7.12 Design System Foundation verified
**License:** MIT

Sergio Knowledge OS (SKOS) is a semantic knowledge platform built for AI-powered information retrieval, management, and exploration.

## Quick Start

```bash
# Install
pip install -e ".[dev]"

# Run current milestone and regression tests
python verify_milestone7_12.py

# Start API
python3 -m uvicorn skos.m6.production.local_server:create_app --factory --host 127.0.0.1 --port 8000
```

After installation, the local operator workflow is available through one command:

```bash
sergio-knowledge prepare
sergio-knowledge status
sergio-knowledge report
sergio-knowledge import /path/to/document.md
sergio-knowledge start
```

## Architecture

SKOS follows Clean Architecture with explicit layer separation:

- **Domain Layer:** Pure business logic, models, and value objects
- **Application Layer:** Orchestration services (embedding, RAG, search, KG)
- **Infrastructure Layer:** Adapters for external concerns (DB, AI providers, API, observability, security)

## API

REST API Contract v1 is frozen and documented at `docs/api_contract.md`.

Key endpoints:
- `POST /api/v1/query` — Unified query (semantic, RAG, graph, hybrid)
- `GET /api/v1/health` — Health check with subsystem status
- `GET /api/v1/status` — System status and version
- `GET /api/v1/engines` — List available engines
- `GET /api/v1/security/status` — Security subsystem status
- `GET /api/v1/admin/overview` — Operator overview for release, readiness and backup
- `GET /api/v1/admin/smoke` — Operator smoke check for core admin readiness
- `GET /api/v1/admin/snapshot` — Single operator snapshot with verdict and next actions
- `GET /api/v1/admin/snapshot/report` — Downloadable plain-text operator snapshot report
- `GET /api/v1/admin/local/launch` — Local launch command and preflight checks
- `POST /api/v1/admin/local/bootstrap` — Prepare local runtime directories safely
- `GET /api/v1/admin/manual` — Structured operator manual for local use
- `GET /api/v1/admin/release` — Current release status for operators
- `POST /api/v1/admin/release/package` — Create a clean release ZIP with manifest and hashes
- `GET /api/v1/admin/release/package/inspect` — Inspect a release ZIP before distribution
- `POST /api/v1/admin/release/gate` — Create, inspect and approve a release package for distribution
- `GET /api/v1/admin/readiness` — Production readiness report
- `POST /api/v1/admin/import/file` — Archive and index a local `.txt`, `.md` or `.json` document
- `POST /api/v1/admin/import/upload` — Upload a document from the local admin console
- `GET /api/v1/admin/ai/status` — Check the configured provider and required AI models
- `GET /admin` — Admin console
- `GET /metrics` — Prometheus metrics
- `GET /api/v1/docs` — Swagger UI
- `GET /api/v1/redoc` — ReDoc

## Security

M4.11 introduces optional security layers:
- API Key Authentication (`x-api-key` or `Authorization: Bearer`)
- RBAC Authorization with wildcard patterns
- Sliding-window Rate Limiting
- Structured Audit Logging

Security is opt-in via configuration. When disabled, the API operates in open mode for backward compatibility.

## Observability

M4.10 introduces production observability:
- Prometheus metrics (`/metrics`)
- OpenTelemetry tracing
- Structured JSON logging
- Request counting and latency histograms

## Milestones

| Milestone | Version | Status |
|:---|:---|:---|
| M2 — Import Engine | 0.2.0 | Frozen |
| M3 — Database Layer | 0.3.0 | Frozen |
| M4.1 — DI & Config | 0.4.0-alpha1 | Frozen |
| M4.2 — Event Bus | 0.4.0-alpha2 | Frozen |
| M4.3 — Embeddings | 0.4.0-alpha3 | Frozen |
| M4.4 — Vector DB | 0.4.0-alpha4 | Frozen |
| M4.5 — Semantic Search | 0.4.0-alpha5 | Frozen |
| M4.6 — RAG Pipeline | 0.4.0-alpha6 | Frozen |
| M4.7 — Knowledge Graph | 0.4.0-alpha7 | Frozen |
| M4.8 — Query Orchestrator | 0.4.0-alpha8 | Frozen |
| M4.9 — REST API | 0.4.0-alpha9 | Frozen |
| M4.9.5 — API Contract Freeze | 0.4.0-alpha11 | Frozen |
| M4.10 — Observability | 0.4.0-alpha12 | Frozen |
| M4.11 — Security & Auth | 0.4.0-alpha13 | Frozen |
| **M4.12 — Release Engineering** | **0.4.0** | **Production** |
| **M5.1 — Persistence Layer** | **0.5.0-alpha1** | **Verified** |
| **M5.2 — Runtime Wiring** | **0.5.0-alpha2** | **Verified** |
| **M5.3 — Admin Console** | **0.5.0-alpha3** | **Verified** |
| **M6.1 — Production Readiness** | **0.6.0-alpha1** | **Verified** |
| **M6.2 — Admin Readiness** | **0.6.0-alpha2** | **Verified** |
| **M6.3 — Backup Manifest** | **0.6.0-alpha3** | **Verified** |
| **M6.4 — Backup Archive** | **0.6.0-alpha4** | **Verified** |
| **M6.5 — Backup Restore Inspection** | **0.6.0-alpha5** | **Verified** |
| **M6.6 — Staged Backup Restore** | **0.6.0-alpha6** | **Verified** |
| **M6.7 — Admin Backup API** | **0.6.0-alpha7** | **Verified** |
| **M6.8 — Admin Backup Console** | **0.6.0-alpha8** | **Verified** |
| **M6.9 — Admin Release Status** | **0.6.0-alpha9** | **Verified** |
| **M6.10 — Admin Overview** | **0.6.0-alpha10** | **Verified** |
| **M6.11 — Admin Smoke Check** | **0.6.0-alpha11** | **Verified** |
| **M6.12 — Release Package Export** | **0.6.0-alpha12** | **Verified** |
| **M6.13 — Release Package Inspection** | **0.6.0-alpha13** | **Verified** |
| **M6.14 — Release Readiness Gate** | **0.6.0-alpha14** | **Verified** |
| **M6.15 — Local Launch Preflight** | **0.6.0-alpha15** | **Verified** |
| **M6.16 — Operator Manual** | **0.6.0-alpha16** | **Verified** |
| **M6.17 — Local Workspace Bootstrap** | **0.6.0-alpha17** | **Verified** |
| **M6.18 — Admin Console UX Polish** | **0.6.0-alpha18** | **Verified** |
| **M6.19 — Operator Snapshot** | **0.6.0-alpha19** | **Verified** |
| **M6.20 — Snapshot Report Export** | **0.6.0-alpha20** | **Verified** |
| **M6.21 — Local Operator CLI** | **0.6.0-alpha21** | **Verified** |
| **M7.1 — AI Runtime Contract** | **0.7.0-alpha1** | **Verified** |
| **M7.2 — Application Runtime Assembly** | **0.7.0-alpha2** | **Verified** |
| **M7.3 — Safe Document Import** | **0.7.0-alpha3** | **Verified** |
| **M7.4 — Admin Document Import** | **0.7.0-alpha4** | **Verified** |
| **M7.5 — Ask Sergio Console** | **0.7.0-alpha5** | **Verified** |
| **M7.6 — Local AI Readiness** | **0.7.0-alpha6** | **Verified** |
| **M7.7 — Local Beta Reliability** | **0.7.0-alpha7** | **Verified** |
| **M7.8 — One-command Local Start** | **0.7.0-alpha8** | **Verified** |
| **M7.9 — Grounded Local Retrieval** | **0.7.0-alpha9** | **Verified** |
| **M7.10 — Control Room Interface** | **0.7.0-alpha10** | **Verified** |
| **M7.11 — Local Database Bootstrap** | **0.7.0-alpha11** | **Verified** |
| **M7.12 — Design System Foundation** | **0.7.0-alpha12** | **Verified** |

## Development

```bash
# Setup
python setup_milestone7_12.py

# Verify
python verify_milestone7_12.py

# Full test suite
pytest -q
```

## Changelog

See [CHANGELOG.md](CHANGELOG.md).

## ADR

See [docs/ADR.md](docs/ADR.md).

## Security Checklist

See [docs/SECURITY_CHECKLIST.md](docs/SECURITY_CHECKLIST.md).

## SBOM

See [SBOM.json](SBOM.json).
