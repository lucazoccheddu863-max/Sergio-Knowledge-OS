#!/usr/bin/env python3
"""Verify script for M6.13 — Release Package Inspection."""
...
def main() -> int:
    print("=" * 60)
    print("M6.13 Verify — Release Package Inspection")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "40 passed" in result.stdout:
        print("  OK M6 tests PASS (40/40)")
    else:
    ...
    if version == "0.6.0-alpha13" and "M6.13" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.13")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking release inspection route wiring...")
    api_text = pathlib.Path("skos/m4/infrastructure/adapters/api/fastapi_adapter.py").read_text(
        encoding="utf-8"
    )
    if "/api/v1/admin/release/package/inspect" in api_text:
        print("  OK release inspection route PASS")
    else:
        print("  FAILED release inspection route missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.13 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
