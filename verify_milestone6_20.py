#!/usr/bin/env python3
"""Verify script for M6.20 — Operator Snapshot Report Export."""
...
def main() -> int:
    print("=" * 60)
    print("M6.20 Verify — Operator Snapshot Report Export")
    print("=" * 60)

    print("\n[1/5] Running M6 tests...")
    result = run([sys.executable, "-m", "pytest", "tests/m6/", "-q"])
    if result.returncode == 0 and "62 passed" in result.stdout:
        print("  OK M6 tests PASS (62/62)")
    else:
    ...
    if version == "0.6.0-alpha20" and "M6.20" in changelog:
        print(f"  OK VERSION = {version}; CHANGELOG includes M6.20")
    else:
        print("  FAILED version or changelog")
        return 1

    print("\n[5/5] Checking snapshot report export wiring...")
    ...
    if "/api/v1/admin/snapshot/report" in api_text and "snapshot-download" in html_text:
        print("  OK snapshot report export wiring PASS")
    else:
        print("  FAILED snapshot report export wiring missing")
        return 1

    print("\n" + "=" * 60)
    print("M6.20 verification complete. All checks PASS.")
    print("=" * 60)
    return 0
